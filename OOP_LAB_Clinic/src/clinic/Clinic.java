...
public class Clinic {
	...
	public void assignPatientToDoctor(String ssn, int docID) throws NoSuchPatient, NoSuchDoctor {
		...
		p.setDoctor(d);
		d.getPatients().add(p);  //se viene ripetuta l'assegnazione abbiamo la duplicazione dei pazienti in un dottore
	}
				...
				if (fields[0].equals("P") ) {
					...
					if (fields.length >= 6) {
						try {
							...
							addDoctor(fields[2], fields[3], fields[4], docID, fields[5]);
	...
	public Collection<Integer> busyDoctors(){
		Double avg = doctors.values().stream()
				.collect(Collectors.averagingDouble(d -> d.getPatients().size()));
		return doctors.values().stream()
	...
	public Collection<String> doctorsByNumPatients(){
		return doctors.values().stream()
				.sorted((a, b) -> {
					return b.getPatients().size() - a.getPatients().size();
				})
